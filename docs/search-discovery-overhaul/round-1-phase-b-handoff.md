...
## CP4 — Header replacement, z-index formalization, Bug 1 layout fix, CP3 v3 cleanup

**Commit:** `<this commit>`. The closing checkpoint of Phase B. Single commit per CONTEXT.md — header component + chrome strip + z-index refactor + Bug 1 fix + scroll-to-top removal + EID §3.2 wording fix all ship as one logical unit. This is a structural pass; piecemeal commits would obscure the net change.

### What shipped

**Phase A interim chrome retired.** The floating top-left logomark/back button, the floating top-right NotificationBell + hamburger cluster, the `pt-14 lg:pt-0` content compensation, the route-planner-era inline form Card, and the collapsed-summary Card with Edit button — all removed from [find-a-wash.tsx](artifacts/washbuddy-web/src/pages/customer/find-a-wash.tsx). Replaced by the EID §3.1 expanded-mode header.

**New file [`components/customer/find-a-wash-header.tsx`](artifacts/washbuddy-web/src/components/customer/find-a-wash-header.tsx).** Self-contained, slots-based, props-only. The host page (find-a-wash.tsx) owns the autocomplete state, the city-option machinery, the route planning handlers, and the placeholder time chip — and passes each row's rendered content as a `React.ReactNode` slot. The header just provides the visual container: sticky positioning at `--z-header`, surface styling (white + backdrop-blur), vehicle-pill row + three-row search card layout. No internal state, no framer-motion shared layoutId — collapsed mode and the layoutId animation wait for Round 2 alongside the bottom sheet whose peek/default/expanded states drive the mode transitions.

**Slots design rationale**: avoids a circular import between the header and find-a-wash.tsx (find-a-wash exports `CityAutocomplete` for the row content; the header imports nothing from find-a-wash). Slots also mean Round 2's bottom sheet can reuse the header inside its expanded state with different slots — the props shape supports that without a refactor.

**z-index hierarchy formalized.** Eleven CSS variables defined at `:root` in [`index.css`](artifacts/washbuddy-web/src/index.css) per EID §3.2:
...
Every existing z-index reference in find-a-wash.tsx refactored to use them:
- Map tile pane: `zIndex: 0` → `var(--z-map)`.
- Map fullscreen overlay: `z-50` → `var(--z-modal)`.
- Map fullscreen toggle button: `z-[401]` → `var(--z-map-control)`.
- "Search this area" pill: `z-30` → `var(--z-map-cta)`.
- Portaled autocomplete dropdown: `z-[1000]` → `var(--z-modal)`. Semantically modal-tier (full-attention overlay rendered into `<body>`).
- New header: `var(--z-header)`.

If a future round introduces non-modal popovers, that's the right time to add a `--z-popover` tier between `--z-header` and `--z-sheet`. Not needed today.

**Bug 1 fix — page now has a bottom.** Root cause: the interim `pt-14 lg:pt-0` content compensation pushed mobile content down by 56px to clear the floating chrome at `top-4`, but didn't shrink the bottom boundary correspondingly. Combined with `flex-1` on AppLayout's children-wrapper and `min-h-screen` on the `<main>` column, the layout column grew indefinitely past content. Removing the `pt-14` AND removing the floating chrome (replaced by the new sticky header in normal document flow) restored a clean content-bottom anchor. Fix is structural, not patched.

**CP3 v3 scroll-to-top removed.** The `window.scrollTo` effect on `searchBoundsAnchor` change was too aggressive — it forced the page to scroll down so the list came into view, displacing the map off-screen. The user's confirmed mental model is silent re-order; the visible signal (button hidden, list re-ordered in place) is sufficient. The `listAnchorRef` declaration is **kept** as forward-compat scaffolding — Round 2's bottom-sheet rebuild may want it for the contained scroll container's own scroll-to-top API. Cheap (one ref, no listener).

**EID §3.2 wording fix.** "The list scrolls to top so the new ordering is visible." was removed; replaced with "The page does not auto-scroll on tap; the visible signal (button hidden, list re-ordered in place) is sufficient." Spec now codifies the user-confirmed behavior.

**Time row placeholder chip.** Read-only chip with a `Clock` icon prefix and a `Lock` icon suffix, styled `bg-slate-50 cursor-not-allowed` with muted text. Reads "Time: now" in nearby mode (no destination) and "Arrive: now" in route mode (destination set). Distinguishes visually from interactive elements so users don't tap expecting interactivity. Round 4 wires the real arrival-time picker.

**Plan Route inline-text affordance.** Replaces the Phase A primary-CTA Plan Route Button. Visible only when both origin and destination are set but no route is currently computed (i.e., user typed free text without committing autocomplete suggestions). Quieter blue-token text-button below the time row; the auto-fire-on-autocomplete-commit pattern handles the typical case without a button being involved.

### Spec sections governing
- [02-eid.md §3.1](docs/search-discovery-overhaul/02-eid.md) — search header expanded mode. CP4 ships expanded mode only; collapsed mode + framer-motion shared layoutId waits for Round 2 alongside the bottom sheet.
- [02-eid.md §3.2](docs/search-discovery-overhaul/02-eid.md) — z-index hierarchy formalized; "Search this area" wording fix shipped.
- [05-visual-reference.md §5.2](docs/search-discovery-overhaul/05-visual-reference.md) — expanded mode visual reference (vehicle pill above three-row search card).

### Verification
- TypeScript: **21 errors**, baseline holds. Zero new in any of the touched files (find-a-wash.tsx, find-a-wash-header.tsx, index.css, EID §3.2).
- Replit canary protocol below.

### Decisions made
- **Slots-based header, not extraction-based.** Extracting `CityAutocomplete` and the city-option machinery into a shared module would have been a ~300-line extraction unrelated to CP4's user-facing scope. Slots achieve the same separation (header is genuinely self-contained, no logic from find-a-wash.tsx leaks in) without the extraction. Round 2 may want to extract if multiple consumers emerge; today, slots suffice.
- **`useScrollDirection` hook file kept; consumer dropped.** No consumer in find-a-wash after the chrome strip. The hook is reusable for Round 2's collapsed-mode header (the same hide-on-scroll-down pattern). Deletion-and-re-add would be churn.
- **`useMobileMenu` consumer dropped from find-a-wash.tsx; context stays in `layout.tsx`.** AppLayout's `MobileMenuContext` is a Phase A permanent (other pages may consume it; AppLayout owns the menu state). Find-a-wash's previous consumer (the now-removed top-right cluster's hamburger trigger) is gone; the new header doesn't trigger the AppLayout menu (it's `hideMobileHeader=true` on this page anyway).
- **`AppLayout.hideMobileHeader` stays `true` on /find-a-wash.** The new sticky header is page-scoped and replaces the AppLayout chrome. Reverting to `false` would create a double-header.
- **Trip metrics strip stays outside `<FindAWashHeader />`.** Rendered by find-a-wash.tsx below the header as page-level decoration. Round 2 can later move metrics into the bottom sheet's peek-state header without touching the header component.
- **Plan Route as a quieter affordance, not removed entirely.** Visual reference §5.2 doesn't show a CTA, but the auto-fire-on-autocomplete-commit pattern doesn't cover manual-text submitters (users who type "350 5th Ave" without picking from the dropdown). The inline-text button preserves that flow at low visual prominence.
- **Bug 1 fix mechanism is structural.** Removing `pt-14` + interim fixed chrome + introducing the sticky header in normal flow naturally re-anchors the layout. No ad-hoc bottom-padding hack.

### Open items for next round (Round 2 — bottom sheet + result cards + pin callout)
- **Bottom sheet introduction** per EID §3.3. Three states (peek / default / expanded), drag-to-snap, mode-aware default state.
- **Collapsed-mode header** per EID §3.1 — the framer-motion shared `layoutId` animation that swaps the expanded card for a single floating pill when the sheet hits peek state. The CP4 expanded header stays as-is; Round 2 wraps it in the layoutId machinery.
- **Result card redesign** per EID §3.4 — new `result-card.tsx` component with rank column (TOP badge / numeric / info icon), service pills, mode-aware metadata line, conditional pricing, gated rating display. Replaces the current `displayLocations.map` cards.
- **Pin callout** per EID §3.6 — replaces the route-planner-era inline `bindPopup` with a dedicated `pin-callout.tsx`.
- **Contained scroll container for the bottom sheet's list** — replaces `window.scrollTo` (already removed in CP4) with the sheet container's own scroll API. `listAnchorRef` is ready.
- **Round 2 inherits from CP4**: the expanded header (stays), the page layout that's ready to introduce a contained scroll container, the z-index hierarchy (`--z-sheet` is already defined for the sheet's tier).

### What's permanent (Phase A + B end state)
- `/find-a-wash` page; redirects from `/search` and `/route-planner` (Phase A).
- `mode = destination ? 'route' : 'nearby'` derivation (Phase A).
- Pin selection model (`selectedLocationId`, `markersByIdRef`, `lastFitKeyRef`) (Phase A).
- `lib/search-helpers.ts`, `lib/normalize-location.ts`, `lib/map-bounds.ts` (Phase A + B).
- `AppLayout.hideMobileHeader` prop + `MobileMenuContext` / `useMobileMenu` hook (Phase A).
- `RedirectTo` helper in App.tsx (Phase A).
- `createPortal` autocomplete dropdown pattern (Phase A).
- `hooks/use-scroll-direction.ts` (Phase A; consumer dropped in CP4 but hook reusable).
- `bboxAround` viewbox helper + `userLat`/`userLng` propagation in `searchPlaces` (Phase A).
- Strict label invariant: lat/lng never in `CityOption.label` (Phase A; `sanitizeMyLocationLabel`).
- `/saved` stub route + page (Phase A).
- Round 0 service taxonomy.
- `wash-pin.tsx` component + classifier (Phase B; tier classification simplified to vehicle-compat-only in CP3 v3).
- `leaflet.markercluster` integration via `clusterGroupRef` (Phase B CP2).
- Vite Leaflet dedupe + `optimizeDeps.include` (Phase B CP3.5; preventive).
- Decimal coercion via `normalizeLocationsResponse` (Phase B Hotfix; load-bearing for marker bounds aggregation).
- "Search this area" button + `searchBoundsAnchor` + `rankedLocations` re-rank + empty-area pill + Show-closest zoom-and-center (Phase B CP3 + variants).
- `find-a-wash-header.tsx` (Phase B CP4).
- z-index hierarchy variables (Phase B CP4).

### Anything that surprised me
- **Slots dissolved the circular-import problem completely.** The header component file imports nothing from find-a-wash.tsx; find-a-wash imports `FindAWashHeader` from the header file. One-way dependency. The autocomplete state machinery, the city-option helpers, the route planning handlers — all stay in find-a-wash and pass through slots as ReactNode. No extraction needed. This pattern is worth remembering for future "extract a sub-component without restructuring its inputs" cases.
- **Bug 1's root cause was less mysterious than expected.** `pt-14 lg:pt-0` + `fixed`-positioned floating chrome + `flex-1`/`min-h-screen` ancestors — three independently-correct choices that combined to extend the page past content. Removing any one alone wouldn't have fixed it cleanly; removing the chrome AND the compensation AND introducing the sticky header in normal flow was the right structural move. CP4's restructure produces the fix as a side effect of the header replacement, not as a targeted patch.
- **The auto-collapse `formCollapsed` effect was Phase A's compromise** for keeping the form Card from dominating screen real estate after a route is planned. With the new header (always-visible expanded form), the compromise becomes irrelevant. The header is sticky, so it doesn't dominate scroll-real-estate the way the inline form Card did. Lesson: some Phase A compromises don't translate forward; they're solving problems the new structure doesn't have.
- **The `listAnchorRef` survives despite its consumer being deleted.** Refs are cheap; the declaration is one line; future consumers (Round 2) avoid a delete-then-re-add. Worth flagging as a pattern: when removing a feature whose forward-compat scaffolding might be reused, keep the scaffolding even if the immediate consumer goes.

### Phase B end state
This is the closing checkpoint of Phase B. After CP4 verifies on Replit, **Phase B is complete.**

Round 2 (bottom sheet + result cards + pin callout) is the next phase of work. Open items above. Round 2 inherits Phase A's structural foundation and Phase B's discovery surface (header, pins, clustering, search-this-area, normalized data layer).

### Replit canary protocol
1. **Header rendering at 320 / 375 / 414 / 768 / 1024 / 1440** — vehicle pill above three-row search card. From → swap → To → When (read-only chip with Clock + Lock icons). Plan Route inline-text affordance below time row, only when both origin and destination are set without an autocomplete commit. No floating chrome.
2. **Bug 1 — page has a bottom.** Scroll to the end of the page on mobile (375px). The result list ends; the layout closes. No infinite scroll.
3. **z-index correctness.** Map controls render below header; "Search this area" pill at `--z-map-cta`; portaled autocomplete dropdown at `--z-modal`; map fullscreen overlay at `--z-modal`. Visual smoke test.
4. **CP3 v3 retained.** "Search this area" tap doesn't repaint map; list re-orders silently; **page does NOT scroll** on tap (the new behavior). Show-closest still zooms-and-centers at zoom 13.
5. **CP1 retained.** Pin tier color reflects vehicle compat (mid-tier light blue for compatible; gray-dashed for incompatible). No top-tier dark blue in default mode.
6. **CP2 retained.** Clustering at zoom <11.
7. **Hotfix retained.** Set destination — no LatLngBounds-recursion crash.
8. **Phase A invariants retained.** Redirects from `/search` and `/route-planner` work; `AppLayout.hideMobileHeader` still suppresses the AppLayout chrome on `/find-a-wash`; portaled autocomplete in the new header works at all viewports.
9. **Time row chip** — visually muted with Clock + Lock icons; doesn't visually invite tapping. Round 4 wires the real picker.
10. **Plan Route affordance** — type a destination as free text without picking from autocomplete; the inline-text "Plan route" appears. Tapping it triggers the route fetch.
