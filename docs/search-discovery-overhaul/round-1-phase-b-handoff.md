...
**Revert commit:** `0f3f876` (reverts `4f2cf77`).

CP4 was reverted because it shipped the EID §3.1 expanded-mode header without the Round 2 bottom sheet the header is designed to live inside. Without an opposing collapsed mode driven by sheet state, the always-expanded header dominated the mobile viewport — ate ~half the screen, reduced the map to ~1 inch tall, stripped the hamburger and notification bell from accessible chrome, and didn't actually resolve Bug 1 (page-has-no-bottom). The header component, the chrome strip, the z-index variable formalization, the `formCollapsed` removal, the CP3 v3 scroll-to-top removal, and the EID §3.2 wording fix all rolled back together as one logical unit. The interim Phase A chrome (floating top-left button, top-right NotificationBell + hamburger cluster, `pt-14 lg:pt-0` compensation, inline form Card + collapsed-summary Card) is back in place; CP1 / CP2 / CP3 v3 / hotfix behaviors are unaffected and continue to work.
